frappe.pages["tally-export"].on_page_load = function (wrapper) {
	new express_tally.tally_export.TallyExportPage(wrapper);
};

frappe.provide("express_tally.tally_export");

express_tally.tally_export.TallyExportPage = class TallyExportPage {
	constructor(wrapper) {
	...
	load_count() {
		...
		frappe.call({
			method: "express_tally.integrations.tally_export.get_sales_order_count",
			args: values,
			callback: (r) => {
	...
	download_masters() {
		...
		if (!params) return;
		window.open(
			`/api/method/express_tally.integrations.tally_export.download_sales_order_masters_json?${params}`,
			"_blank",
		);
	}
	...
	download_sales_orders() {
		const params = this.get_download_params();
		if (!params) return;
		window.open(`/api/method/express_tally.integrations.tally_export.download_sales_order_json?${params}`, "_blank");
	}
};
