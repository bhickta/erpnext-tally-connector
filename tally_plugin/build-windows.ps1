...
$OutputZip = Join-Path $RepoRoot "dist\ERPNext-Tally-Bridge-Windows-x64.zip"
...
$Executable = Join-Path $ExeDir "ERPNextTallyBridge.exe"
& $Executable --help
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
Set-Content -Path $ChecksumFile -Value "$Hash  ERPNext-Tally-Bridge-Windows-x64.zip" -Encoding ascii
