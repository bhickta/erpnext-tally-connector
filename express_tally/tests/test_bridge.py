...
class TestSyncService(TestCase):
	def setUp(self):
		self.config = BridgeConfig(
			...
			flow_name="srv.sales_documents_to_tally",
		)
	...
	def test_unknown_agent_profile_is_rejected_before_import(self):
		frappe_client = Mock()
		frappe_client.get_unsynced_documents.return_value = {
			"schema_version": 1,
			"flow": "srv.sales_documents_to_tally",
			"agent_profile": "purchase_voucher_v1",
			"documents": [sample_order()],
		}
		tally_client = Mock()
		tally_client.get_current_company.return_value = "Tally Company"
		...
		result = SyncService(self.config, frappe_client, tally_client).sync_once()
...
class TestFrappeFlowClient(TestCase):
	def setUp(self):
		...
		self.client = FrappeClient("https://erp.example.com", "key", "secret")
		self.client.request = Mock(return_value={})

	def test_client_uses_the_framework_api(self):
		config = replace(
			self.config,
			to_date="2026-08-31",
		)

		self.client.get_unsynced_documents(config)
		path = self.client.request.call_args.args[1]
		params = self.client.request.call_args.args[2]
		self.assertEqual(path, "/api/method/express_tally.framework.api.pull")
		self.assertEqual(params["flow"], "srv.sales_documents_to_tally")
		self.assertEqual(params["to_date"], "2026-08-31")

		self.client.acknowledge(config, [{"request_id": "request-1"}])
		path = self.client.request.call_args.args[1]
		data = self.client.request.call_args.args[2]
